...
def linear_scale(y_pred: jnp.ndarray, y_true: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    y_pred_mean = jnp.mean(y_pred)
    y_true_mean = jnp.mean(y_true)
    a = jnp.sum((y_pred - y_pred_mean) * (y_true - y_true_mean))
    a /= jnp.sum((y_pred - y_pred_mean) ** 2)
    b = y_true_mean - a * y_pred_mean
    y_scaled = a * y_pred + b
    return y_scaled, a, b
...
if __name__ == "__main__":
    ...
    for seed in range(10):
        for problem in problems:
            ...
            conf["run_name"] = (
                    "CGP_linscal_" + conf["problem"].replace("/", "_") + "_" + str(conf["seed"])
            )
            print(conf["run_name"])
