...
def get_d4rl_dataset(env_name="invertedpendulum/expert-v0"):
    ...
    for episode in dataset.iterate_episodes():
        ...
        obs = obs[:-1]  # drop terminal extra state
...
def run_sym_reg_ga(config: Dict):
    X, y = get_d4rl_dataset()
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=500, random_state=config["seed"]
    )
    ...
    # print(n_features)
    n_features = max(jnp.round(jnp.sqrt(X_train.shape[1])).astype(int), 6)
    ls = config["ls"]
...
if __name__ == "__main__":
    ...
    for seed in range(10):
        for ls in [True, False]:
            ...
            conf["ls"] = ls
            conf["run_name"] = (
                    f"CGP_feats{ls_text}_" + "IL_" + str(conf["seed"])
            )
