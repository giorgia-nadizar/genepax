...
def feature_construction_scoring_fn(genotypes: Genotype, key: RNGKey, X_train: jnp.ndarray, y_train: jnp.ndarray,
                                    X_test: jnp.ndarray, y_test: jnp.ndarray, cgp_structure: CGP,
                                    inner_fn=single_genome_feature_construction_scoring_fn
...
def run_sym_reg_ga(config: Dict):
    ...
    # print(n_features)
    n_features = int(np.round(np.sqrt(X_train.shape[1])))
    if n_features < 2:
        return
    ...
    sequential_gp = SequentialGP(first_cgp_structure, second_cgp_structure)

    # Init the population
    key, subkey = jax.random.split(key)
    init_keys = jax.random.split(key, config["n_pop"])
    init_population = jax.jit(jax.vmap(sequential_gp.init))(init_keys)
    ...
    # Define emitter
    mutation_fn = jax.jit(jax.vmap(sequential_gp.mutate, in_axes=(0, 0)))
    ...
    inner_scoring_fn = single_genome_feature_construction_scoring_fn
    inner_rescoring_fn = single_genome_feature_construction_rescoring_fn
    scoring_fn = partial(
        feature_construction_scoring_fn,
        X_train=X_train_sub, y_train=y_train_sub,
        X_test=X_test, y_test=y_test, cgp_structure=sequential_gp,
...
if __name__ == "__main__":
    ...
    for seed in range(10):
        for problem in problems:
            ...
            conf["n_gens"] = n_gens
            conf["run_name"] = (
                    f"CGP_seq_" + conf["problem"].replace("/", "_") + "_" + str(conf["seed"])
            )
            print(conf["run_name"])
